...
from resistDBParser_v0_2 import CResistDB
...
class ResistType(object):
    ...
    def preFilterStep(self):
        ...
        cmdLine = '{0} mem {1} {2} {3} -O 9 -B 3  | python {4}/src/filterUnmappedReadpairs.py filteredFastq/{5}/reads1.fq.gz filteredFastq/{5}/reads2.fq.gz '.format(self.bwaPath, self.resistGeneFastaFullPadded, self.fastqFile1, self.fastqFile2, _baseDir, self.suffix)
        logger.info(cmdLine)
        os.system(cmdLine)
    ...
    def getFastaSeqs(self):
        ...
        os.system(cmdLine)
        cmdLine = "java -jar {0}  R={1}/reference.fa O={1}/reference.dict".format(self.picardToolsPath, self.outputDir)
        os.system(cmdLine)
        cmdLine= "{0} genomecov -ibam {1}/output.bam -dz > {1}/outputBam.cov".format(self.bedtoolsPath, self.outputDir)
        os.system(cmdLine)
        ...
        cmdLine = "java -jar {0} -T FastaAlternateReferenceMaker     -R {1}/reference.fa     -o   {1}/output.fasta   -V {1}/output.vcf     -IUPAC  {1}/output.bam -L {1}/gatkIntervals.list ".format(self.gatkPath, self.outputDir)
        os.system(cmdLine)
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--percentIdentity", dest="percentIdentity", type="float", default=90, help="percent identity threshold")
    parser.add_option("-C", "--coverage", dest="coverage", type="float", default=0.9, help="blast overlap coverage threshold")
    ...
    resistTypeModule= ResistType([opts.refid, opts.sampleName, opts.assemblyFile, opts.batchName, opts.metaGenomics, opts.percentIdentity, opts.coverage])
