...
class mlstType(object):
    def __init__(self, args):
        ...
        self.outDir = "mlst/{0}/".format(self.sampleid)
        cmdLine = "rm -rf "+ self.outDir
        os.system(cmdLine)
        cmdLine = "mkdir -p " + self.outDir
        os.system(cmdLine)
        self.miniContigFile = self.outDir + "mlst.contigs.fa"
        self.blastFile = self.outDir + "outputBlast.txt"
        self.mlstOutFile = self.outDir + "mlst_new.txt"
        self.matchedAlleles = []
        ...
        self.spadesPath= _baseDir + "/bin/spades.py"
            ...
            if len(alleleCounter) ==7 and len(alleleTypeA) ==7:
                ...
                if hasST==0:
                    if lenMostCommonAlleleGroup >1:
                        f.write("{0},mixedST,{1}\n".format(self.sampleid, ",".join(sorted(alleleTypeA))))
                    else:
                        f.write("{0},newST_newAllele,{1}\n".format(self.sampleid, ",".join(sorted(alleleTypeA))))
    ...
    def runMapAndSpades(self):
        ...
        if self.fq2 != None:
            fqs.append(self.fq2)
        filterScript= _baseDir + "/scripts/filterUnmappedReadpairs.py"
        self.miniFq1 = self.outDir + "mlst_reads1.fq.gz"
        self.miniFq2 = self.outDir + "mlst_reads2.fq.gz"
    ...
    def getMatchedAlleles(self):
        ...
        self.matchedAlleleFile = self.outDir + "matchedAlleles.txt"
...
if __name__ == "__main__":
    ...
    mlstModule= mlstType([opts.sampleName, opts.contigFile, opts.fq1, opts.fq2])
    mlstModule.runMLST()
