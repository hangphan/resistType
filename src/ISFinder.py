...
class ISFinder(object):
    def __init__(self, args):
        self.args = args
        self.sampleid = args[0]
        self.isdbDir= _baseDir + "/resources/isdb/"
        self.alleleDB= self.isdbDir + "ISFinderDatabase.fasta"
        self.contigFile = args[1]

        self.outDir = "isfinderOutput/{0}/".format(self.sampleid)
        cmdLine = "rm -rf "+ self.outDir
        os.system(cmdLine)
        cmdLine = "mkdir -p " + self.outDir
        os.system(cmdLine)
        self.blastFile = self.outDir + "outputBlast.txt"
        self.outFile = self.outDir + "outISFinder.txt"
        self.closestMatchedAlleles=[]
        self.matchedAlleles=[]
        
    def runISFinder(self):
        self.runBlast()
...
if __name__ == "__main__":
    ...
    ISFinderModule= ISFinder([opts.sampleName, opts.contigFile])
    ISFinderModule.runISFinder()
