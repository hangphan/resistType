...
class CResistDB(object):
    ...
    def getResistanceGeneFeature(self, geneName):
        ...
        return None
    def readGenotypeFile(self, fileName):
        if not os.path.exists(fileName):
            print "{0} not exist. Quit here".format(fileName)
            #resistType,/Batch1/Kenya_1,CTX-M-15,100.0,100.0,None,1.66,0,NODE_36_length_5846_cov_67.0575_ID_7373|2259|3134|40.3295307124|4.9570707443|dnaMatch
        for line in open(fileName, "r"):
            cols=line.strip().split()
            method, sampleName, gene, coverage, pid, isTruncated, copyNumber, matchingInfo =cols[:8]
    ...
    def resistancePrediction(self, exactMatchList, mismatchDic, species, foN, sampleName, genotypeFile):
        ...
        for gene in sorted(mismatchDic.keys()):
            ...
            if gene.startswith('TEM') and not isTruncatedProtein :#and pident>0.95 :
                hasTEM=1
            if gene.startswith('ampC')  and not isTruncatedProtein:# and pident >0.95:
                hasAmpC=1
